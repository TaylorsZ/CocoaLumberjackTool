...
+ (instancetype)logItemWithLevel:(NSString *)level name:(NSString *)name timeString:(NSString *)timeString logString:(id)logString {
    ...
    NSDictionary<NSAttributedStringKey, id> *timeAttributes = [CocoaLumberjackTool appearance].timeAttributes;
    ...
    NSDictionary<NSAttributedStringKey, id> *textAttributes = [CocoaLumberjackTool appearance].textAttributes;
...
- (void)updateDisplayStringWithSearchResults:(NSArray<NSTextCheckingResult *> *)searchResults {
    ...
    [searchResults enumerateObjectsUsingBlock:^(NSTextCheckingResult * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        [displayString addAttribute:NSBackgroundColorAttributeName value:[[CocoaLumberjackTool appearance].searchResultHighlightedBackgroundColor colorWithAlphaComponent:.4] range:NSMakeRange(self.timeString.length + obj.range.location, obj.range.length)];
    }];
...
- (void)focusSearchResultAtIndex:(NSInteger)index {
    ...
    NSMutableAttributedString *displayString = self.displayString.mutableCopy;
    [displayString addAttribute:NSBackgroundColorAttributeName value:[CocoaLumberjackTool appearance].searchResultHighlightedBackgroundColor range:range];
    self.displayString = displayString.copy;
}

@end

@interface QMUIConsoleLogItemCell : UITableViewCell
...
- (void)didInitialize {
    ...
    self.backgroundColor = [CocoaLumberjackTool appearance].backgroundColor;
...
- (QMUIButton *)popoverButton {
    if (!_popoverButton) {
        ...
        _popoverButton.backgroundColor = [[CocoaLumberjackTool appearance].backgroundColor colorWithAlphaComponent:.5];
